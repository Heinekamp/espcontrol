...
The **Clock Bar** section controls whether the top bar is shown. It also contains the indoor and outdoor temperature controls shown in that bar, including whether temperatures include the `°` suffix. When the clock bar is enabled, the small network icon at the far right shows WiFi strength or Ethernet status. Tap it to see the device name, IP address, WiFi strength, uptime, and firmware version.
